...
export async function generateOutreachAction(leadId: string): Promise<ActionResult> {
  try {
    const { supabase, user, workspaceId } = await authorized();
    const { data: lead } = await supabase.from('leads').select('id,name,role,company_id').eq('workspace_id', workspaceId).eq('id', leadId).maybeSingle();
    const { data: company } = lead?.company_id ? await supabase.from('companies').select('id,name,domain,research_status,research_output,research_updated_at').eq('workspace_id', workspaceId).eq('id', lead.company_id).maybeSingle() : { data: null };
    if (!lead || !company?.research_output) throw new Error('Research is required before outreach generation');
    const provider = createAIProviderFromEnv();
    const service = new OutreachService(provider, new SupabaseOutreachDraftStore(supabase), { record: async (ws, run) => { const persisted = Object.fromEntries(Object.entries(run as Record<string, unknown>).filter(([key]) => key !== 'durationMs')); await supabase.from('ai_runs').insert({ workspace_id: ws, ...persisted, operation: 'outreach_generation' }); } });
    const draft = await service.generate({ workspaceId, leadId, companyId: String(company.id), lead: { name: String(lead.name), role: lead.role ? String(lead.role) : null }, company: { name: String(company.name), domain: String(company.domain ?? '') }, research: { ...(company.research_output as Record<string, unknown>), status: String(company.research_status), version: String(company.research_updated_at), researchedAt: String(company.research_updated_at) } } as never);
    await supabase.from('activities').upsert({ workspace_id: workspaceId, lead_id: leadId, company_id: company.id, actor_kind: 'operator', actor_user_id: user.id, activity_type: 'outreach_generated', effect_key: `outreach_generated:${draft.id}`, details: { draft_id: draft.id, version: draft.version } }, { onConflict: 'workspace_id,effect_key', ignoreDuplicates: true });
    revalidatePath(`/leads/${leadId}`); revalidatePath('/outreach');
    return { ok: true };
